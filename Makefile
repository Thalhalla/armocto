...
run: tmp steam_username steam_password steam_guard_code builddocker rundocker beep

rundocker:
	@docker run --name=armocto \
	--cidfile="cid" \
	-v /tmp:/tmp \
...
builddocker:
	/usr/bin/time -v docker build -t thalhalla/armocto .
